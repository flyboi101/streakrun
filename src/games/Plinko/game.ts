...
export class Plinko {
  ...
  constructor(props: PlinkoProps) {
    ...
    // Define the horizontal offset to align pegs to the left
    const horizontalOffset = this.width * 0.25; // Adjust this value to control left alignment
    ...
    const pegs = Array.from({ length: this.props.rows })
      .flatMap((_, row) => {
        ...
        return Array.from({ length: cols }).map((_, column) => {
          ...
          const x = horizontalOffset + colSpacing * column;
          const y = rowSize * row + rowSize / 2;
          return Matter.Bodies.circle(x, y, PEG_RADIUS, {
            isStatic: true,
            label: "Peg",
            plugin: { pegIndex: row * (row + 1) / 2 + column },
          });
        });
      });
